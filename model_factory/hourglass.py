...
class HourglassNet(nn.Module):
    ...
    def __init__(self, block, num_stacks=2, num_blocks=4, num_classes=14, BN=True, num_G=16,
                 train_spread=False):
        print(f"Number of stacks: {num_stacks}")
        print(f"Stack depth: {num_blocks}")  # This parameter is actually not used in original code, Hourglass depth is hardcoded to 4
        ...
        # Initial feature extraction part
        self.inplanes = 64
        self.num_feats = 128
        ...
        # build hourglass modules and connecting layers
        ch = self.num_feats * block.expansion  # 256

        # Create num_stacks Hourglass modules
        self.hg = nn.ModuleList([
            Hourglass(block, num_blocks, self.num_feats, 4, BN, num_G) for i in range(self.num_stacks)
        ])

        # Final estimation head, only used once after all stacks complete
        self.estimator = build_EstimationHead(3, input_dim=256, depth_dim=64, num_classes=num_classes,
                                              train_spread=train_spread, BN=BN, num_G=num_G)
    ...
    def forward(self, x):
        ...
        # Pass through each stacked Hourglass sequentially
        for i in range(self.num_stacks):
            # Current stack's hourglass
            hg_out = self.hg[i](x)


        # At this point fc_out is the output feature map from the last stack
        # Pass final feature map to estimation head
        UVD, f1, f2, f3, f4, f5, f6, Joints = self.estimator(hg_out)

        return UVD, f1, f2, f3, f4, f5, f6, Joints
...
if __name__ == '__main__':
    NET = HourglassNet(Bottleneck, num_stacks=2, num_blocks=2, num_classes=16,BN=True,num_G=16, train_spread=False)
    img = torch.randn((1,1,128,128))
    output,f1,f2,f3,f4,f5,f6,_ = NET(img)
    print(output.shape)