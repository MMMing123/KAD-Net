...
def get_optimizer(optimizer_type, model, args):
    no_decay = [] if args.decay_bias else ['bias', 'bn']  # no_decay is the list of parameters excluded from weight decay, defaults to empty
    grouped_parameters = [
...
def get_scheduler(optimizer, args):
    ...
    if scheduler_type == "steplr":
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=args.num_epoch,
                                                                         T_mult=args.Tmult, eta_min=args.eta_min)
    elif scheduler_type == "auto":
        scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, "min", patience=args.patience, min_lr=1e-8,
...
## Get dataset class (DataSets)
def DATA_Getters(args):
    # cubic_size=[280,280,280]
    # Shear size, also the input depth image size: cropSize;
    # image_size = (128,128)
    cubic_size = [args.cubic_size, args.cubic_size, args.cubic_size]
    image_size = (args.cropSize, args.cropSize)

    ## Initialize basic parameters required for model
    # do_norm_zero_one whether to normalize features to [0,1], otherwise normalize to [-1,1]
    base_parameters = dict(basepath=args.datasetpath, train=True, cropSize=image_size,
                           doJitterRotation=args.RotAugment, sigmaNoise=args.sigmaNoise,
                           rotationAngleRange=[-45.0, 45.0], comJitter=args.comjitter,
                           RandDotPercentage=args.RandDotPercentage,
                           cropSize3D=cubic_size, do_norm_zero_one=False,
                           random_seed=args.randseed, drop_joint_num=args.drop_joint_num,
    ...
    if args.dataset == "nyu":
        ...
        # camID view number; basepath dataset path;
        specs = dict(camID=args.camid, basepath=os.environ.get('NYU_PATH', args.datasetpath))
        # Update base parameters, as each dataset requires different parameters
        ...
        specs = dict(basepath=os.environ.get('MSRA_PATH', args.datasetpath), LeaveOut_subject=args.leaveout_subject,
...
## Calculate loss based on mask, loss for each keypoint (average, excluding zeros)
# Input: predicted uvd, ground truth uvd, mask, L1 loss function
# Returns: average L1 loss
def loss_masked(preds, GRM_Joints, target, mask, lossFunc, f1, f2, f3, f4, f5, f6):
    # preds, targets: both tensor of shape (B,num_joint,k)
    # mask a tensor of shape (B,num_joint,1)
    bs = preds.shape[0]
    joint_dim = preds.shape[2]

    ## !!! Split labels target_Plam[bs,1] others[bs,3]
    target_Plam = target[:, 0:1, 2]
    target_Thumb = target[:, 1:4, 2]
    target_Index = target[:, 4:7, 2]
    target_Middle = target[:, 7:10, 2]
    target_Ring = target[:, 10:13, 2]
    target_Pinky = target[:, 13:16, 2]

    mask_Plam = mask[:, 0:1, 0]
    mask_Thumb = mask[:, 1:4, 0]
    mask_Index = mask[:, 4:7, 0]
    mask_Middle = mask[:, 7:10, 0]
    mask_Ring = mask[:, 10:13, 0]
    mask_Pinky = mask[:, 13:16, 0]

    ## Sub-task losses
    ## First calculate losses: f1[bs,1] others[bs,3] target_Plam[bs,1] others[bs,3]
    loss_f1 = lossFunc(f1, target_Plam)
    loss_f2 = lossFunc(f2, target_Pinky)
    loss_f3 = lossFunc(f3, target_Ring)
    loss_f4 = lossFunc(f4, target_Middle)
    loss_f5 = lossFunc(f5, target_Index)
    loss_f6 = lossFunc(f6, target_Thumb)

    ## Then apply mask and sum
    loss_f1 = torch.sum(loss_f1 * mask_Plam)
    loss_f2 = torch.sum(loss_f2 * mask_Pinky)
    loss_f3 = torch.sum(loss_f3 * mask_Ring)
    loss_f4 = torch.sum(loss_f4 * mask_Middle)
    loss_f5 = torch.sum(loss_f5 * mask_Index)
    loss_f6 = torch.sum(loss_f6 * mask_Thumb)

    loss_num = torch.sum(mask)

    loss_fn = loss_f1 + loss_f2 + loss_f3 + loss_f4 + loss_f5 + loss_f6
    loss_fn = loss_fn / loss_num
    ...
    uv_loss = lossFunc(preds_uv, target_uv)  # # B,num_joints,2
    d_loss = lossFunc(preds_d, target_d)  # # B,num_joints,1

    distance = torch.cat((uv_loss,d_loss), dim=2)  # # B,num_joints,3
    # Apply mask: set loss for invisible points to 0
    distance = distance * mask

    # Calculate effective non-zero loss count: = mask count * 3
    num_noneZero = torch.sum(mask) * joint_dim

    # If no valid joints, set num_noneZero to 1 to prevent division by zero error
    if num_noneZero == 0:
        num_noneZero = 1

    ## GRM loss
    GRMloss = lossFunc(GRM_Joints, target_uv)  # B,num_joints,2
    loss_num = torch.sum(mask) * 2
    GRMloss = torch.sum(GRMloss * mask)  # [B,num_joints,2] * [B,num_joints,1]
    GRMloss = GRMloss / loss_num

    # Return weighted average loss
    return torch.sum(distance) / num_noneZero + 0.2*loss_fn + 0.5 * GRMloss
...
class AverageMeter(object):
    ...
    def update(self, data: dict):
        for key in data:
            self.data[key] = self.data.get(key, 0) + data[key].detach()
...
def Grad_Updater(loss, model, optimizer, fp16_scaler, args):
    ...
    if fp16_scaler is None:
        loss.backward()
        if args.clip_max_norm > 0:  # Gradient clipping (not used by default)
...
## Save model
# Save contains "model": model parameters; "args": training arguments, "scheduler": learning rate scheduler
def make_checkpoint(chk_name, model, optimizer, scheduler, args):
    data = {"model": (model.module.state_dict() if not args.paralelization_type == "N" else model.state_dict()),
            "args": args, "scheduler": scheduler.state_dict()}  # "optimizer":optimizer.state_dict(),
...
## Load pth training model from checkpoint
...
## Overwrite args with config file
...
def model_builder(model_name, num_joints, args):
    # Get first part and convert to lowercase
    arc_name = model_name.split("_")[0].lower()
    if arc_name == "hglass":
        from model_factory.hourglass import HourglassNet, Bottleneck
        # Following is number of stacks
        num_stack = int(model_name.split("_")[2].lower())
        num_blocks = int(model_name.split("_")[1].lower())
        return HourglassNet(Bottleneck, num_stacks=num_stack, num_blocks=num_blocks, num_classes=num_joints, BN=True,
                            num_G=16, train_spread=args.train_spread)
    # Raise exception
    print("error: model_name should call hglass")
    raise NotImplementedError
