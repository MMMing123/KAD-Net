...
class HandposeEvaluation(object):
    ...
    def getMeanError(self):
        """
            Calculate average distance error for all keypoints in all batches (in mm)
            :return: mean error (scalar)
        """
        #  np.nanmean function ignores NaN values in array, only calculates mean for non-NaN elements
        #  np.sqrt(np.square(self.gt - self.joints).sum(axis=2))===> shape: [bs,num_key]
        #  NOTE: two nanmean
    ...
    def getErrorPerDimension(self,printOut=True):
        """
           Get average error in each direction for all batches and all joints (mm)
        """
        d=np.abs(self.gt-self.joints)
